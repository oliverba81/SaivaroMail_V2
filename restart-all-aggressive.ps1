...
REM Beende alle Node-Prozesse im Projekt
taskkill /F /IM node.exe /T >nul 2>&1

REM Warte 2 Sekunden
timeout /t 2 /nobreak >nul

REM (1) Port-Pruefung: Falls Ports noch belegt, kurz nachwarten
powershell -NoProfile -Command "$ports = 3000,3001,3002; $inUse = @(Get-NetTCPConnection -LocalPort $ports -ErrorAction SilentlyContinue | Where-Object { $_.State -eq 'Listen' }); if ($inUse.Count -gt 0) { Write-Host 'Port(s) noch belegt - warte 3 Sekunden...'; Start-Sleep -Seconds 3 }"
...
echo URL: http://testfirma2.localhost:3000
...
echo Warte auf Mailclient (Port 3000)...
timeout /t 5 /nobreak >nul
powershell -NoProfile -Command "try { $ok = Test-NetConnection 127.0.0.1 -Port 3000 -InformationLevel Quiet -WarningAction SilentlyContinue; if ($ok) { Write-Host 'OK: Mailclient laeuft auf Port 3000' -ForegroundColor Green } else { Write-Host 'Hinweis: Port 3000 noch nicht erreichbar - ggf. Fenster pruefen' -ForegroundColor Yellow } } catch { Write-Host 'Server-Status nicht pruefbar' -ForegroundColor Gray }"
echo.
